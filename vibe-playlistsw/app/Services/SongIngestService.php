...
class SongIngestService
{
    ...
    public function ingest(string $urlOrId, ?int $userId = null): Song
    {
        ...
        // 4) Upsert song (⚠️ stamp owner)
        $song = Song::updateOrCreate(
            ['spotify_id' => $trackId],
            [
                'user_id'         => $uid,                         // <—— owner for scoping
                'created_by'      => $uid,                         // keep your legacy column too
                'name'            => $meta['name'] ?? null,
        ...
        // Remove existing mood/activity tags, then attach fresh
    ...
    public function ingestMany(array $trackIds, ?int $userId = null): int
    {
        ...
        // Batch fetch
        $tracks       = $this->spotify->tracks($trackIds);                   // array of tracks
        $featuresById = $this->spotify->audioFeaturesMany($trackIds);        // id => features

        // Build meta map; collect artist ids
        $metaById  = [];
        $artistIds = [];
        foreach ($tracks as $t) {
